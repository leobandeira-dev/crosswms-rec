...
export const PacotesTableFixed = ({ searchTerm, onEdit }: PacotesTableProps) => {
  ...
  const toggleStatusMutation = useMutation({
    mutationFn: async (pacoteId: string) => {
      return apiRequest(`/api/admin/pacotes/${pacoteId}/toggle-status`, {
        method: 'PATCH',
      });
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || "Ocorreu um erro ao alterar o status do pacote.",
        variant: "destructive",
      });
    },
  });

  const filteredPacotes = pacotes.filter((pacote: any) =>
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          <p className="text-red-600 font-medium">Erro ao carregar pacotes</p>
